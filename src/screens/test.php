...
if( CModule::IncludeModule("iblock") ){
	...
	AddEventHandler('main', 'OnBeforeEventSend', Array("Medc2Event", "OnBeforeEventSendHandler"));
	
	class Medc2Event{
		function OnBeforeEventSendHandler(&$arFields, $arTemplate){
			if(isset($arFields['SPECIALIST']) && $arFields['SPECIALIST']){
				$arElement = CCache::CIblockElement_GetList(array('CACHE' => array('TAG' => CCache::GetIBlockCacheTag(CCache::$arIBlocks[SITE_ID]["aspro_medc2_content"]["aspro_medc2_staff"][0]), 'MULTI' => 'N')), array('IBLOCK_ID' => CCache::$arIBlocks[SITE_ID]["aspro_medc2_content"]["aspro_medc2_staff"][0], 'ID' => $arFields['SPECIALIST']), false, false, array('NAME'));
				$arFields['SPECIALIST'] = $arElement['NAME'];
			}
	...
	if( $bCache && $obFormCache->InitCache( $arParams["CACHE_TIME"], $CACHE_ID, $CACHE_PATH ) ){
		...
		if(isset($arParams["SET_NAME_AND_PREVIEW"]) && $arParams["SET_NAME_AND_PREVIEW"] == "Y"){
		    $arResult["arQuestions"][] = array(
			"NAME" => GetMessage("TEXT_FOR_NAME"), 
			"CODE" => "NAME_FOR_ELEMENT",
			"PROPERTY_TYPE" => "S",
			"USER_TYPE" => NULL,
			"IS_REQUIRED" => "Y"
			);
		    $arResult["arQuestions"][] = array(
			"NAME" => GetMessage("TEXT_FOR_MESSAGE"), 
			"CODE" => "MESSAGE_FOR_PREVIEW",
			"PROPERTY_TYPE" => "S",
			"USER_TYPE" => "HTML",
			"IS_REQUIRED" => "Y"
			);
		...
		while( $arProp = $rsProp->Fetch() ){
			if(isset($arParams["PROPS_TO_FILL"]) && is_array($arParams["PROPS_TO_FILL"]) && !empty($arParams["PROPS_TO_FILL"])){
			    if(!in_array($arProp["CODE"], $arParams["PROPS_TO_FILL"])){
				continue;
			    }
			}
		...
		\Bitrix\Main\Loader::includeModule('aspro.medc2');
		$arModuleOptions = CMedc2::GetFrontParametrsValues(SITE_ID);
		...
		foreach( $arResult["QUESTIONS"] as $FIELD_CODE => $arQuestion ){
			...
			switch( $arQuestion["FIELD_TYPE"] ){
				...
				case "integer":
					...
					break;
				case "date":
			$html = '<input type="text" id="'.$arQuestion["CODE"].'" name="'.$arQuestion["CODE"].'" class="form-control date wcalendar '.$required.'" '.$placeholder.' value="'.$val.'" />'.$icon;
					break;
				case "datetime":
					$html = '<input type="text" id="'.$arQuestion["CODE"].'" name="'.$arQuestion["CODE"].'" class="form-control datetime '.$required.'" '.$placeholder.' value="'.$val.'" readonly />'.$icon;
					break;
				case "html":
			...
			if( count( $arResult["FORM_ERRORS"] ) <= 0 ){
				//if( check_bitrix_sessid() ){
					...
					$arFields = array(
						...
						"ACTIVE" => ($arParams["CREATE_DEACTIVATED"] == "Y") ? "N" : "Y",
						"NAME" => ($arParams["SET_NAME_AND_PREVIEW"] == "Y") ? $arPropFields["NAME_FOR_ELEMENT"] : GetMessage("DEFAULT_NAME").ConvertTimeStamp(),
						"PREVIEW_TEXT" => ($arParams["SET_NAME_AND_PREVIEW"] == "Y") ? $arPropFields["MESSAGE_FOR_PREVIEW"]["VALUE"]["TEXT"] : "",
					...
					if( $RESULT_ID = $el->Add( $arFields ) ){
						...
						if(strlen($arEventFields["EMAIL"])){
							if(isset($arEventFields['ORDER_LIST']) && strpos('aspro_medc2_order_page', $arResult['IBLOCK_CODE']) !== false){
